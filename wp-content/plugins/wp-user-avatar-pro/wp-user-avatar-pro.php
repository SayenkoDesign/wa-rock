	...
	class WP_User_Avatar_Pro
	{
		...
		function _init() {
			...
			function processor() {
				...
				try {
					...
					$factoryObject = new FactoryControllerPMP();
			...
			function create_menu() {
				...
				$pagehook1 = add_options_page(
					__( 'WP User Avatar', WPUAP_TEXT_DOMAIN ),
					__( 'WP User Avatar', WPUAP_TEXT_DOMAIN ),
					'wpuap_manage_settings',
					WPUAP_SLUG,
				...
				if ( current_user_can( 'manage_options' )  ) {
								...
								$role->add_cap( 'wpuap_manage_settings' );
				}
		 ...
		 */
		    ...
		    public function wpua_editor_enquque_scripts(){
			    ...
			    if (  get_option( 'wp_user_avatar_hide_webcam',true ) != 1 ) {
				    $display_webcam = 'true';
			    } else {
			    ...
			    if ( current_user_can( 'upload_files' ) and get_option( 'wp_user_avatar_hide_mediamanager',true ) != 1 ) {
			    ...
			    $string = array(
				    ...
				    'save' => __( 'Choose Image',WPUAP_TEXT_DOMAIN ),
			...
			private function _define_constants() {
				...
				if ( ! defined( 'WPUAP_SLUG' ) ) {
					define( 'WPUAP_SLUG', 'wpuap_manage_settings' );
				}

				if ( ! defined( 'WPUAP_VERSION' ) ) {
					define( 'WPUAP_VERSION', '4.0.3' );
				}
			...
			private function _load_files() {
				...
				foreach ( $files_to_include as $file ) {
					require_once( WPUAP_CORE_CLASSES.$file );
				}
				// Load all modules.
				$core_modules = array( 'settings' );
