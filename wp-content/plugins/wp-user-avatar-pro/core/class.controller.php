...
if ( ! class_exists( 'WPUAP_Core_Controller' ) ) {
	...
	class WPUAP_Core_Controller {
	    ...
	    function __construct($objectType) {
			...
			if ( file_exists( WPUAP_MODEL.$this->entity.'/model.'.$this->entity.'.php' ) ) {
				$factoryObject = new FactoryModelPMP();
		...
		public function display($view, $options = array()) {
			...
			if ( ! empty( $view ) ) {
				return include( WPUAP_MODEL. "{$this->entity}/views/".$view );
			}
		}
		/**
		 * Return entity name.
		...
		protected function action_add_edit() {
	    	...
	    	if ( $this->entityObj->save() > 0 ) {
				...
				if ( isset( $_GET['doaction'] ) and 'edit' == $_GET['doaction'] ) {
					$response['success']  = __( $current_obj_name.' updated successfully.',WPUAP_TEXT_DOMAIN );
				} else { 					$response['success']  = __( $current_obj_name.' added successfully. You can manage your '.$current_obj_name.'s <a href="'.admin_url( 'admin.php?page=WPUAP_manage_'.$this->entity ).'">here</a>.', WPUAP_TEXT_DOMAIN ); }
