...
if ( ! class_exists( 'FlipperCode_HTML_Markup' ) ) {
	...
	class FlipperCode_HTML_Markup {
		 ...
		 * @var Array
		 */
		private $form_elements = array( 'text','checkbox','multiple_checkbox','radio','submit','button','select', 'hidden', 'wp_editor', 'html', 'datalist','textarea' , 'file' , 'div' , 'blockquote','html' , 'image','group','table', 'message', 'anchor', 'image_picker' );
		/**
		 * Attributes Allowed
		 * @var Array
		 */
		...
		public function __construct( ) {
			...
			$this->allowed_attributes = array_fill_keys( array( 'vertical','lable', 'id', 'class', 'required', 'default_value', 'value', 'options', 'desc', 'before', 'after', 'radio-val-label', 'onclick', 'placeholder', 'textarea_rows', 'textarea_name', 'html', 'current', 'width', 'height', 'src', 'alt', 'heading', 'data', 'show' ) , '' );
		...
		public function get_manage_url() {
			return "<a style='float:right' target='_blank' href='http://codecanyon.net/downloads'>Rate Me - <img src='".WPUAP_IMAGES."rating.png' /></a>";
		}
		/**
		...
		public function get_header() {
			...
			return apply_filters( 'WPUAP_form_header_html', $output );
		}
		/**
		 * Form footer getter.
		...
		public function get_footer( ) {
			...
			return apply_filters( 'WPUAP_form_footer_html', $output );
		}
		/**
		...
		public function get_combined_markup() {
			...
			if ( $this->elements ) {
				...
				$before = apply_filters( 'WPUAP_element_before_start_row', '<div class="form-group {modifier}">' );
				$after = apply_filters( 'WPUAP_element_after_end_row', '</div>' );
				$num = 0;
		...
		public static function get_element_html( $name, $type, $atts ) {
			...
			if ( 'hidden' == $type ) {
				...
				if ( ! empty( $atts['lable'] ) ) {
					$element_output .= apply_filters( 'WPUAP_input_label_'.$name, '<div class="col-md-3"><label for="'.$name.'">'.$atts['lable'].'</label>'.self::element_mandatory( @$atts['required'] ).'</div>' ); }
		...
		public static function field_image_picker($name, $atts) {
			...
			$html .= Responsive_Markup::field_anchor('ci_choose_image', array(
				'value' => __( 'Choose Marker Image', WPUAP_TEXT_DOMAIN ),
				'href' => 'javascript:void(0);',
				'class' => 'btn btn-info ci_choose_image col-md-3 ',
				'data' => array( 'target' => $name ),
			));

			$html .= Responsive_Markup::field_anchor('remove_image', array(
				'value' => __( 'Remove Marker Image', WPUAP_TEXT_DOMAIN ),
		...
		public static function field_text( $name, $atts ) {
			...
			return apply_filters( 'WPUAP_input_field_'.$name, $element, $name, $atts );
		}
		/**
		...
		public static function field_select( $name, $atts ) {
			...
			if ( isset( $atts['desc'] ) && ! empty( $atts['desc'] ) ) {
				$element .= '<p class="help-block">'.$atts['desc'].'</p>'; }
			return apply_filters( 'WPUAP_select_field_'.$name, $element, $name, $atts );
		}
		...
		public static function field_multiple_checkbox( $name, $atts ) {
			...
			if(is_array($value)) {
				...
				$element  .= '<div class="checkbox-inline"><input type="checkbox"  name="'.$name.'" value="'.esc_attr( stripcslashes( $key ) ).'"'.self::get_element_attributes( $atts ).' '.checked( $val, $atts['current'], false ).'/>'.$val.'</div> ';
				}
			}
			return apply_filters( 'WPUAP_input_field_'.$name, $element, $name, $atts );
		...
		public  static function field_radio( $name, $atts ) {
			...
			$radio_style = $atts['vertical'] ? 'radio' : 'radio-inline';
			
			if ( is_array( $atts['radio-val-label'] ) ) {

				foreach ( $radio_options as $radio_val => $radio_label ) {
					$element .= '<label class="'.$radio_style.'"><input type="radio" name="'.$name.'" value="'.esc_attr( stripcslashes( $radio_val ) ).'"'.self::get_element_attributes( $atts ).' '.checked( $radio_val, $elem_value, false ).'>&nbsp;&nbsp;'.$radio_label.'</label>';
				}
			}
			return apply_filters( 'WPUAP_input_field_'.$name, $element, $name, $atts );
		...
		public static function field_table($name, $atts) {
			...
			if ( is_array( $heads ) ) {
				...
				foreach ( $heads as $head ) {
					$output .= '<td><strong>'.__( $head, WPUAP_TEXT_DOMAIN ).'</strong></td>';
				}
			}
			...
			return apply_filters( 'WPUAP_input_field_'.$name, $output, $name, $atts );
		...
		public static function button($title, $link) {
			...
			return '<span class="glyphicon glyphicon-add WPUAP_new_add button action"><a href="'.esc_html( $link ).'">'.__( $title,WPUAP_TEXT_DOMAIN ).'</a></span>';
		}
