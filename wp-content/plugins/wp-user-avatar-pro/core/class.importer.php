...
if ( ! class_exists( 'FlipperCode_Export_Import' ) ) {
	...
	class FlipperCode_Export_Import {
		...
		*/
		...
		function export($action, $asFilename) {
			...
			if ( 'csv' == $action ) {
				...
				if ( ! empty( $this->data ) ) {
					$WPUAP_tab = "\t";
					$WPUAP_br = "\n";
					$WPUAP_xml_writter  = '<?xml version="1.0" encoding="UTF-8"?>'.$WPUAP_br;
					$WPUAP_xml_writter .= '<items>'.$WPUAP_br;

					foreach ( $this->data as $key => $result ) {
						$WPUAP_xml_writter .= $WPUAP_tab.'<item>'.$WPUAP_br;
						foreach ( $result as $node_key => $node_value ) {
							$WPUAP_xml_writter .= $WPUAP_tab.$WPUAP_tab.'<'.$node_key.'>'.htmlspecialchars( stripslashes( $node_value ) ).'</'.$node_key.'>'.$WPUAP_br;
						}
						$WPUAP_xml_writter .= $WPUAP_tab.'</item>'.$WPUAP_br;
					}

					$WPUAP_xml_writter .= '</items>';
					echo $WPUAP_xml_writter;
				}
			} elseif ( 'json' == $action ) {
		 ...
		 */
		public function WPUAP_xml_2array($xml) {
	        ...
	        foreach ( $xml->children() as $r ) {
	            ...
	            if ( count( $r->children() ) == 0 ) {
	                ...
	                $arr[ $r->getName() ][] = $this->WPUAP_xml_2array( $r );
	            }
	        }
		...
		function import($action, $filename) {
			...
			if ( 'csv' == $action ) {
				...
				$WPUAP_xml_datas = simplexml_load_file( $_FILES[ $filename ]['tmp_name'] );
				$file_data = $this->WPUAP_xml_2array( $WPUAP_xml_datas );
				...
				$file_data = array();
				$WPUAP_json_datas = wp_remote_fopen( $_FILES[ $filename ]['tmp_name'] );
				if ( false === $WPUAP_json_datas ) {
					$WPUAP_json_datas = file_get_contents( $_FILES[ $filename ]['tmp_name'] );
				}
				$file_datas = json_decode( $WPUAP_json_datas, true );
			}
