jQuery(document).ready(function($) {
    ...
    $("body").on('change', "input[name='avatar_default']", function() {
        var value = $(this).val();
        if (value == 'wp_user_avatar_custom_url') {
            $('.wp_custom_image_url').parent().removeClass('hiderow');
        } else {
            $('.wp_custom_image_url').parent().addClass('hiderow');
        }
    });
    $("body").on('change', "input[name='avatar_storage_option']", function() {
        var value = $(this).val();
        $('.wp_storage_directory, .wp_storage_aws, .wp_storage_dropbox').parent().addClass('hiderow');

        if (value == 'media') {
            $('.wp_storage_directory, .wp_storage_aws, .wp_storage_dropbox').parent().addClass('hiderow');
        } else if (value == 'directory') {
            $('.wp_storage_directory').parent().removeClass('hiderow');

        } else if (value == 'aws') {
            $('.wp_storage_aws').parent().removeClass('hiderow');

        } else if (value == 'dropbox') {
            $('.wp_storage_dropbox').parent().removeClass('hiderow');
    ...
    $('span.delete a').click(function() {
        ...
        if (confirm(WPUAP_js_lang.confirm))
            return true;
    ...
    $("div.WPUAP_form_horizontal .WPUAP_add_more_fields").click(function() {

        var WPUAP_length = $(".form-group").length + 1;

        var more_textbox = $('<div class="row form-group">' +
            '<div class="col-md-8"><input  placeholder="Enter name here..." class="form-control" type="text" name="name[]" id="txtbox' + WPUAP_length + '" /></div>' +
            '<div class="col-md-2"><a href="javascript:void(0);" class="btn btn-danger btn-xs WPUAP_remove_more_fields">Remove</a>' +
            '</div></div>');
    ...
    $('div.WPUAP_form_horizontal').on('click', '.WPUAP_remove_more_fields', function() {
        ...
        if (remove == true) {
            ...
            jQuery.ajax({
                type: "POST",
                url: WPUAP_local.urlforajax,
                data: {
                    action: 'WPUAP_ajax_operation',
                    'id': id
                },
                beforeSend: function() {},
    ...
    $('.remove_image').click(function() {
        WPUAP_image_id = $(this).parent().parent();
        $(WPUAP_image_id).find('.selected_image').attr('src', '');
        $(WPUAP_image_id).find('input[name="' + $(this).data('target') + '"]').val('');
        $(this).hide();
        return false;
    });

    $(".WPUAP_check_backup").click(function() {
        var WPUAP_bid = confirm("Overwrite existing google maps database?.");
        if (WPUAP_bid == true) {
    ...
    $(".WPUAP_search_input").keyup(function() {
        map_id = $(this).attr("rel");
        $(".WPUAP_locations_listing[rel='" + map_id + "']").addClass("WPUAP_loading");
        WPUAP_filter_locations(map_id, 1);
    });

    $(".WPUAP_toggle_container").click(function() {
        $(".WPUAP_toggle_main_container").toggle("slow");
        if ($(this).text() == "Hide") {
            $(this).text("Show");
        } else {
            $(this).text("Hide");
    ...
    $(".WPUAP_mcurrent_loction").click(function() {
        WPUAP_get_current_location();
    });
    ...
    var maptable = $('#WPUAP_google_map_data_table').dataTable({
        "lengthMenu": [
    ...
    var route_maptable = $('#WPUAP_google_map_route_data_table').dataTable({
        "lengthMenu": [
            [10, 25, 50, 100, 200, 500, -1],
            [10, 25, 50, 100, 200, 500, "All"]
        ],
...
function send_icon_to_map(imagesrc, target) {
    WPUAP_image_id = jQuery("#" + target);
    jQuery(WPUAP_image_id).find('.ci_remove_image').show();
    jQuery(WPUAP_image_id).find('img.selected_image').attr('src', imagesrc);
    var img_hidden_field = jQuery(WPUAP_image_id).find('.ci_choose_image').data('target');
    jQuery(WPUAP_image_id).find('input[name="' + img_hidden_field + '"]').val(imagesrc);
